...
func enableCORS(ctx *fasthttp.RequestCtx) {
	...
	if string(ctx.Method()) == "OPTIONS" {
		...
		ctx.SetContentType("application/json")
		ctx.SetBody([]byte(`{"message":"API funcionando correctamente"}`))
		return // Maneja preflight CORS
...
func getNombre(ctx *fasthttp.RequestCtx, ps fasthttprouter.Params) {
	enableCORS(ctx)

	nombre := strings.TrimSpace(ps.ByName("nombre"))

	if nombre == "" {
		ctx.Error("Nombre inválido", fasthttp.StatusBadRequest)
		return
	}
	...
	if len(servicios) == 0 {
		ctx.Error("No encontrado", fasthttp.StatusNotFound)
		return
	}

	jsonResponse(ctx, fasthttp.StatusOK, servicios)
}
...
func main() {
	...
	router.GET("/servicios/nombre/:nombre", getNombre)
